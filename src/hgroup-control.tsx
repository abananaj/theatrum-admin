...
const NATIVE_SELECT_SELECTOR = ".block-editor-block-inspector__advanced select.components-select-control__input"
...
addFilter("editor.BlockEdit", "chance/add-hgroup-toggle", (BlockEdit: any) => {
	return (props: any) => {
		...
		useEffect(() => {
			...
			const nativeSelect = document.querySelector<HTMLSelectElement>(NATIVE_SELECT_SELECTOR)
			if (!nativeSelect) return

			const looksLikeHtmlElementSelect = Array.from(nativeSelect.options).some((option) => option.value === "header")
			if (!looksLikeHtmlElementSelect) return
		...
		const onToggle = (value: boolean) => {
			setAttributes({ tagName: value ? "hgroup" : previousTagName.current || DEFAULT_TAG_NAME })
		}
