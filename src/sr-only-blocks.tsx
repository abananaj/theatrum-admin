					...
					el(
						...
						el(ToggleControl, {
							...
							help: srOnly
								...
								: 'Make this content visible only to screen readers',
							checked: srOnly,
							onChange: (value: boolean) =>
								setAttributes({ srOnly: value }),
						}),
						srOnly
							? el(Notice, {
									status: 'info',
									isDismissible: false,
									children:
										'The sr-only class will be applied to hide this content visually.',
								})
							: null
					)
				)
...
addFilter(
	...
	(BlockListBlock: any) => {
		return (props: any) => {
			...
			return el(
				'div',
				{ className: 'wp-block-sr-only-wrapper' },
				el(BlockListBlock, props),
				el(
					'div',
					{
						className: 'wp-block-sr-only-badge',
						'aria-hidden': 'true',
					},
					'SR-ONLY'
				)
